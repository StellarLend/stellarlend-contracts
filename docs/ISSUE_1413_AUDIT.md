...
stellar-lend/contracts/amm/src/lib.rs:28: //! - Flash‑swap APIs (e.g. `flash_swap_a_for_b`, `repay_flash_swap`) and a dedicated `AmmPoolErr
                (sole hit is the rustdoc note itself; the
                 ...
                 at any SHA on this branch. The actual rustdoc
                 line continues past column 110 with `or type`;
                 the listing above is generated by the explicit
                 `cut -c1-110` so what is shown matches what the
                 command emits by construction. Drop the cut to
                 see the full line.)

$ grep -cE '^[[:space:]]*pub fn' stellar-lend/contracts/amm/src/lib.rs
5

$ grep -nE '^[[:space:]]*pub fn' stellar-lend/contracts/amm/src/lib.rs
