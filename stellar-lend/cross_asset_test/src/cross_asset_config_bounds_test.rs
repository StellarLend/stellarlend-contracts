...
fn test_update_rejects_ltv_above_100_pct() {
    ...
    with_contract(&env, || {
        let admin = Address::generate(&env);
        set_admin(&env, &admin);
        // Use a high threshold so that the factor-bounds check fires first.
        initialize_asset(
            &env,
            None,
            AssetConfig {
...
/// `price_decimals` of zero is a misconfiguration: every price would be
/// interpreted as integer units, silently scaling values by up to 10^18.
#[test]
...
/// `price_decimals` above 38 exceeds i128 range and must be rejected with
/// `InvalidDecimals`.
#[test]
...
fn test_update_valid_persists_changes() {
    ...
    with_contract(&env, || {
        ...
        let r = update_asset_config(
            ...
            &admin,
            None,
            Some(6_000), // factor 60 %
            Some(9_000), // threshold 90 %
            Some(1_000_000),
            Some(500_000),
            Some(false),
            Some(true),
...
/// A `None`-only update (all optional fields absent) must succeed and leave
...
/// Existing assets not touched by `update_asset_config` retain their
...
fn test_update_preserves_other_assets() {
    ...
    with_contract(&env, || {
        ...
        // `other_asset` config is unchanged.
