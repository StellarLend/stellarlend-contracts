...
use soroban_sdk::{
    ...
    Address, Env, Vec, IntoVal, Val, symbol_short
};
...
impl Grant {
    pub fn vested_at(&self, effective_now: u64) -> i128 {
        ...
        // Partitioned division to avoid u128 intermediate multiplication overflow
...
impl VestingContract {
    ...
    pub fn resume(env: Env, caller: Address) -> Result<(), VestingError> {
        ...
        let new_total = total_paused.checked_add(interval).ok_or(VestingError::Overflow)?;
    ...
    pub fn transfer_grant(env: Env, caller: Address, from: Address, to: Address) -> Result<(), VestingError> {
        Self::require_admin(&env, &caller)?;
        Self::require_not_paused(&env)?;

        let mut from_grants: Vec<Grant> = env
            .storage()
            .persistent()
            .get(&VestingKey::Grant(from.clone()))
            .ok_or(VestingError::GrantNotFound)?;

        if env.storage().persistent().has(&VestingKey::Grant(to.clone())) {
            let to_grants: Vec<Grant> = env.storage().persistent().get(&VestingKey::Grant(to.clone())).unwrap();
            if to_grants.len() > 0 {
        ...
        for i in 0..from_grants.len() {
            let mut grant = from_grants.get(i).unwrap();
            ...
            from_grants.set(i, grant);
        }

        env.storage().persistent().remove(&VestingKey::Grant(from.clone()));
        env.storage().persistent().set(&VestingKey::Grant(to.clone()), &from_grants);
        ...
        let topics = (soroban_sdk::Symbol::new(&env, "grant_transferred"), from.clone());
        let mut data: Vec<Val> = Vec::new(&env);
        data.push_back(from.clone().into_val(&env));
        data.push_back(to.clone().into_val(&env));
        env.events().publish(topics, data);
