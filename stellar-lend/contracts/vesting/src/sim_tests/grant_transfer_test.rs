...
fn test_sim_grant_transfer_unauthorized() {
    ...
    vc.add_grant("admin", "alice", 1_000, 0, 1_000, 0).unwrap();
    let err = vc
        .transfer_grant("attacker", "alice", "bob")
        .unwrap_err();
...
fn test_sim_grant_transfer_grant_not_found() {
    let mut vc = VestingContract::new("admin", "treasury");
    let err = vc
        .transfer_grant("admin", "nonexistent", "bob")
        .unwrap_err();
