...
fn test_fee_calculation_default() {
    ...
    let result = env.as_contract(&contract_id, || {
        execute_flash_loan(&env, user.clone(), token_address.clone(), 10_000_000, callback)
    });

    assert!(result.is_ok());
    // 10M * 9 / 10000 = 9000
    assert_eq!(result.unwrap(), 10_009_000);
}

/// Test custom fee (50 bps)
#[test]
fn test_fee_calculation_custom() {
    ...
    assert!(result.is_ok());
    // 1M * 50 / 10000 = 5000
    assert_eq!(result.unwrap(), 1_005_000);
}
...
fn test_unpaid_loan_revert() {
    ...
    // Attempt repayment with insufficient amount
    let result = env.as_contract(&contract_id, || {
        repay_flash_loan(&env, user.clone(), token_address.clone(), total - 100)
    });

    assert_eq!(result.unwrap_err(), FlashLoanError::InvalidAmount);
}

/// Test no active loan revert
#[test]
fn test_no_active_loan_revert() {
    let (env, contract_id, _admin, user, token_address) = setup_env();

    let result = env.as_contract(&contract_id, || {
        repay_flash_loan(&env, user, token_address, 1_000_000)
    });

    assert_eq!(result.unwrap_err(), FlashLoanError::NoActiveFlashLoan);
}
...
/// Test invalid callback address (contract itself)
#[test]
