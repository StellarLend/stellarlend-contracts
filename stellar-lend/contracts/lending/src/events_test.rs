...
fn test_schema_version_event_emitted_on_initialize() {
    ...
    assert_eq!(
        schema_events.len(),
        1,
        "Should emit exactly one SchemaVersionEvent on initialize"
    );
}
...
fn test_deposit_event_emitted() {
    ...
    assert_eq!(
        deposit_events.len(),
        1,
        "Should emit exactly one DepositEvent"
    );
}
...
fn test_deposit_event_contains_correct_data() {
    ...
    assert!(
        deposit_event.data.clone().into_val(&env).is_ok(),
        "Event data should be valid"
    );
}
...
fn test_withdraw_event_emitted() {
    ...
    assert_eq!(
        withdraw_events.len(),
        1,
        "Should emit exactly one WithdrawEvent"
    );
}
...
fn test_withdraw_full_balance_emits_event() {
    ...
    assert_eq!(
        withdraw_events.len(),
        1,
        "Should emit WithdrawEvent even for full withdrawal"
    );
}
...
fn test_borrow_event_emitted() {
    ...
    assert_eq!(
        borrow_events.len(),
        1,
        "Should emit exactly one BorrowEvent"
    );
}
...
fn test_full_repay_emits_event_with_zero_debt() {
    ...
    assert_eq!(
        repay_events.len(),
        1,
        "Should emit RepayEvent for full repayment"
    );
}
...
fn test_deposit_at_cap_boundary_emits_event() {
    ...
    if result.is_ok() {
        ...
        assert!(
            deposit_events.len() > 0,
            "Should emit DepositEvent at cap boundary"
        );
    }
}
...
fn test_failed_operation_does_not_emit_event() {
    ...
    assert!(
        result.is_err(),
        "Withdrawal should fail with insufficient balance"
    );
