...
use crate::borrow::{
    get_liquidation_close_factor, get_liquidation_incentive, get_user_collateral,
    get_user_debt, save_collateral_position, save_debt_position,
    get_total_debt, set_total_debt, BorrowError,
};
use crate::views::{get_health_factor, debt_value, collateral_value, HEALTH_FACTOR_SCALE};
...
/// * `debt_asset` - Asset being repaid
/// * `collateral_asset` - Asset being seized
/// * `repay_amount` - Amount of debt tokens the liquidator is providing
///
/// # Security
/// * Re-verifies health factor < 1.0 (10000 bps)
/// * Enforces Close Factor (e.g. max 50% repayment)
/// * Calculates incentive-adjusted collateral seizure
pub fn liquidate(
    env: &Env,
    liquidator: Address,
    borrower: Address,
    ...
    let seized_value = I256::from_i128(env, actual_repay_value)
        .mul(&I256::from_i128(env, 10000 + incentive))
        .div(&I256::from_i128(env, 10000))
        .to_i128()
        .ok_or(BorrowError::Overflow)?;

    let collateral_to_seize = I256::from_i128(env, collat_pos.amount)
        .mul(&I256::from_i128(env, seized_value))
        .div(&I256::from_i128(env, cv))
        .to_i128()
        .ok_or(BorrowError::Overflow)?;

    // Caps seizure to available collateral
