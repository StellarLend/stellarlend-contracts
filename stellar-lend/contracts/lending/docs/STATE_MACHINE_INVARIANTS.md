...
8. [Operation Record Lifecycle and Cancellation](#operation-record-lifecycle-and-cancellation)
9. [Failure Recovery Strategies](#failure-recovery-strategies)
...
## Operation Record Lifecycle and Cancellation

### Operation Record States

| State | Meaning | Persisted Result |
|-------|---------|------------------|
| `None` | No operation registered for this `operation_id` | Not persisted |
| `Pending` | Intent registered, not yet executed | Operation intent and deadline |
| `Executing` | Transaction is applying mutations | Only visible inside the executing transaction |
| `Completed(result)` | Terminal success; result is immutable and replay-safe | Cached result |
| `Failed(error)` | Terminal failure from a pre-mutation or post-rollback rejection | Error code |
| `Cancelled` | Terminal client-initiated cancellation; does not block a new operation_id | Cancellation marker |

### Success, Rejection, Cancellation, and Retry Transitions

| From | To | Trigger | Invariants Enforced |
|------|----|---------|---------------------|
| `None` | `Pending` | Client registers an operation intent | TTL set; no balance/debt mutation |
| Any (no record) | Unchanged | Pre-mutation rejection (`InvalidAmount`, pause, auth failure) | No operation record created; no state mutation; sequence unchanged |
| `Pending` | `Executing` | Transaction enters mutating section | Single writer per operation_id; concurrent duplicate returns `OperationInProgress` |
| `Executing` | `Completed(result)` | All post-conditions pass | Cached result is immutable; sequence increments exactly once |
| `Executing` | `Failed(error)` | Any pre-condition or health check fails after rollback | No balance/debt mutation remains; sequence unchanged |
| `Pending` | `Cancelled` | Client calls `cancel_operation` before execution | Only `Pending` can be cancelled; `Completed` and `Failed` are terminal |
| `Failed` | `Pending` | Client retries with same operation_id after corrective action | Old error preserved until overwrite; retry cannot replay a `Completed` operation |
| `Cancelled` | `Pending` | Client re-registers same operation_id | Only allowed after explicit cancellation; new TTL assigned |
| `Completed(result)` | `Completed(result)` | Duplicate submission with same operation_id | Returns cached result; never mutates state or increments sequence again |

### Stale Response and Duplicate Submission Reconciliation

Clients MUST reconcile uncertain submissions before retrying:

1. If `operation_id` resolves to `Completed(result)`, return that cached result and do not resubmit.
2. If `operation_id` resolves to `Pending` or `Executing`, query `get_user_sequence()` and the relevant balance/debt key:
   - Sequence advanced and state changed → operation succeeded; treat as `Completed`.
   - Sequence unchanged and state unchanged → operation did not execute; safe to retry.
3. If no `operation_id` was used, state inspection is the only recovery path. Compare current balance/debt with the expected post-operation value.
4. Never retry automatically after `InvalidAmount`, `SelfLiquidation`, or `RepayAmountTooHigh` until the caller has corrected the parameters.
5. A retry after an ambiguous timeout MUST use a fresh `operation_id` if the previous `operation_id` status cannot be determined, so the retry cannot be mistaken for a duplicate of an already-executed action.
